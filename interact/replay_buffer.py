...
class ReplayBuffer(object):
    def __init__(self, root_dir, max_length):
        ...
        self.scalar_keys = ['type', 'action', 'pos_action', 'reward', 'move_flag', 'score']
    ...
    def load(self, other_root_dir):
        ...
        if self.max_length != self.scalar_data['max_length']:
            raise ValueError(
                'max length should be cosistent when loading replay buffer')
        ...
        if other_root_dir != self.root_dir:
            ...
            shutil.copytree(os.path.join(
                other_root_dir, 'tensor_data'), self.tensor_dir)
    ...
    def save_data(self, data_dict):
        ...
        f = h5py.File(os.path.join(self.tensor_dir,
                      'tensor_data-%d.hdf5' % (self.point)), 'w')
    ...
    def fetch_data(self, indexes):
        ...
        for key in self.scalar_keys:
            return_scalars[key] = np.array(
                [self.scalar_data[key][idx] for idx in indexes])
